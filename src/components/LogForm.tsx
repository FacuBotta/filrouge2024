...
type LogFormProps = {type: string, onClick?: () => void}

export default function LogForm({type, onClick}: LogFormProps) {
  ...
  const handleUrlChange = (e: React.MouseEvent) => {
    e.preventDefault();
    const href = type === 'Sign-In' ? '/signup' : '/login';
    router.replace(href);
  };
